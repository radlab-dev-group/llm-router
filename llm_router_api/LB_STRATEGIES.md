...
## Hot‑Reload (config changes on-the-fly)

When the router is configured with `LLM_ROUTER_CONFIG_SOURCE=etcd` (see [ETCD.md](ETCD.md)), load-balancing weights,
provider lists, and even the set of active models can be changed without restarting any container. All routers pick up
the new config within ~100 ms via the etcd watcher API.

This means you can:

* **Add a provider** — it immediately becomes available for selection by any strategy.
* **Change weights dynamically** — `weighted` and `dynamic_weighted` strategies react instantly to weight changes in the
  config.
* **Drain providers** — remove or set `weight=0` for maintenance; requests redistribute automatically.

When using file-based config, hot-reload is not available — you must redeploy for changes to take effect.
