...
| `LANGCHAIN_RAG_COLLECTION`                            | Name of the FAISS collection used by the LangChain RAG plugin. If unset, the collection will be `None` and the plugin will raise an error when instantiated.                                                                                                             | `None`                                 |
| `LANGCHAIN_RAG_EMBEDDER`                              | Hugging Face model identifier or local path for the sentence‑embedding model. If unset, it will be `None` and cause an error on plugin creation.                                                                                                                         | `None`                                 |
| `LANGCHAIN_RAG_DEVICE`                                | Torch device on which the embedding model runs (`cpu`, `cuda:0`, etc.).                                                                                                                                                                                                  | `"cpu"`                                |
| `LANGCHAIN_RAG_CHUNK_SIZE`                            | Number of tokens per chunk when splitting texts.                                                                                                                                                                                                                         | `400`                                  |
| `LANGCHAIN_RAG_CHUNK_OVERLAP`                         | Number of overlapping tokens between consecutive chunks.                                                                                                                                                                                                                 | `100`                                  |
